...
public interface IThesisTitleDao {
	...
	ThesisTitle findById(int id);
	
	ThesisTitle getThesisById(int id);
	
	/*int updateThesisTitle(ThesisTitle thesisTitle);*/
	
	@Update("update t_thesis_title set thesisName=#{thesisName},teacherId=#{teacherId},submitTime=#{submitTime},status=#{status},inputMan=#{inputMan},nandu=#{nandu},from=#{from},liang=#{liang},leixing=#{leixing},description=#{description},state =#{state} where id=#{id}")
	void updateThesisTitle(ThesisTitle thesisTitle);
